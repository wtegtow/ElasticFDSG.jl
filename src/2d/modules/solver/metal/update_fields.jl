# =============================================================================
# update fields metal kernel
# =============================================================================

function update_normal_kernel!(
    N, c, dt,
    ny, nx, dy, dx, 
    c11,c13,c33,
    vx,vy,
    ...
    y,x    = Tuple(thread_position_in_grid_2d())
    stride = Tuple(threads_per_grid_2d())

    while 2+(N-1) <= y <= ny-(N-1) &&
          1+(N-1) <= x <= nx-N
...
function update_vx_kernel!(N, c, dt,
    ny, nx, dy, dx, 
    rho,
    sxx,sxy,
    vx,
...
function update_vy_kernel!(N, c, dt,
    ny, nx, dy, dx, 
    rho,
    sxy,syy,
    vy,
