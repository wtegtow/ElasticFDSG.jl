...
function update_normal_stresses_cuda(
    ...
    z = threadIdx().x + (blockIdx().x - 1) * blockDim().x
    y = threadIdx().y + (blockIdx().y - 1) * blockDim().y
    x = threadIdx().z + (blockIdx().z - 1) * blockDim().z
